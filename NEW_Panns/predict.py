"""
PANNs Prediction Script for Heart Sound Classification.
...
from model.dataset import extract_panns_features, PANNS_SAMPLE_RATE, PANNS_TARGET_DURATION
from common.audio import load_audio, apply_bandpass_filter, normalize_audio, pad_or_truncate


class PANNsPredictor:
    """Predictor class for PANNs heart sound classification."""
    ...
    def __init__(
        ...
        self.model = create_model(
            model_type=config.get('model_type', 'cnn14'),
    ...
    def preprocess_audio(self, audio_path: str) -> torch.Tensor:
        ...
        target_length_4k = int(PANNS_TARGET_DURATION * 4000)
        audio = pad_or_truncate(audio, target_length_4k)
        
        audio_32k = librosa.resample(audio, orig_sr=4000, target_sr=PANNS_SAMPLE_RATE)
        features = extract_panns_features(audio_32k, sr=PANNS_SAMPLE_RATE)
...
def main():
    parser = argparse.ArgumentParser(description='PANNs Heart Sound Prediction')
    ...
    predictor = PANNsPredictor(args.model, args.device)
